...
Lifecycle handling needs a durable store so payment creation and the eventual webhook, which happen in different requests and often different processes, agree on state. PostgreSQL is the published adapter.
...
It runs the shared conformance suite plus SDK-level lifecycle checks: handler retry after a failure, no concurrent re-run on lease takeover, and duplicate acknowledgement.

<Callout>Adapters for MongoDB, MySQL, and Cassandra exist in the repository but are not published yet, because their queries have not run against a real engine. Use PostgreSQL, or write a store and hold it to the [conformance suite](/docs/conformance).</Callout>
